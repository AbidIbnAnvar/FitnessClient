...
function UserFitness() {
  ...
  async function deleteData(workout){
    ...
    const response = await fetch("http://localhost:8080/user/delete-workout",{
      method:'DELETE',
    ...
    const post_response= await axios.get('http://localhost:8080/user/get-workout/', {withCredentials: true}).then(res=>{
    ...
    <div className='user-fitness'>
      ...
      <div class='workout-main'>
        ...
        <div className='workout-data'>
          ...
          {(workoutData)?
       workoutData.map((workout,index)=><p className='each-workout-data'><div className="post-no">{index+1}</div><div>{workout.workout_name}</div><div>{(workout.workout_reps)?<>{workout.workout_reps} reps done </>:<div className='workout-duration-data'>for {workout.workout_duration} s</div> }</div><div>{workout.workout_calories} Cal burnt</div><div className="post-username">{workout.date}</div><button className='delete-data' onClick={(e)=>{e.preventDefault();deleteData(workout);setDisplayData(true)}}><i className="fa-solid fa-trash"></i></button></p>)
       :<p></p>}</>:<button onClick={()=>{changeDisplayData();display();}} className='display-workouts'><i class="fa-solid fa-angle-down"></i>Display workouts
          </button>} 
